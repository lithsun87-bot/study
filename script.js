...
function animateCount(el){
  ...
  let start=0;const dur=1400;const step=dur/target;
...
function updateMissionProgress(){
  const tasks=document.querySelectorAll('#missionTasks .task-item');
  const done=document.querySelectorAll('#missionTasks .task-item.done').length;
  const pct=Math.round((done/tasks.length)*100);
  document.getElementById('missionProg').style.width=pct+'%';
  document.getElementById('missionProgText').textContent=`${done} / ${tasks.length} งานเสร็จ`;
}
...
function buildWeekGrid(){
  ...
  const dayMap=[6,0,1,2,3,4,5];// map index to weekDay (Sun=0 in JS)
...
function initCharts(){
  ...
  // Streak heatmap (simulated as bar)
...
function updateTimerDisplay(){
  ...
  document.getElementById('timerProg').style.width=pct+'%';
}
...
function updatePomoDots(){
  const dots=document.querySelectorAll('#pomoDots .pomo-dot');
  dots.forEach((d,i)=>{
